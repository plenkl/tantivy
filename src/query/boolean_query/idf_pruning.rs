...
mod tests {
    ...
    /// End-to-end test: IDF pruning activates when QueryParser has set_idf_pruning(true).
    ///
    ...
    /// QueryParser -> TermQuery(Basic) -> TermWeight(Basic) -> SkipFreq -> idf_pruning
    #[test]
    fn test_idf_scoring_end_to_end() -> crate::Result<()> {
        ...
        // IDF-only path: set_idf_pruning skips frequencies
    ...
    fn test_idf_scoring_with_freqs_index() -> crate::Result<()> {
        ...
        // IDF-only path: set_idf_pruning skips frequencies on a WithFreqs index
        let mut qp_idf = QueryParser::for_index(&index, vec![body]);
        qp_idf.set_idf_pruning(true);
        let query_idf = qp_idf.parse_query("cat dog")?;
        let idf_results = searcher.search(&query_idf, &TopDocs::with_limit(5).order_by_score())?;
        ...
        assert!(
            ...
            "WithFreqs index with IDF pruning should return results"
        );
